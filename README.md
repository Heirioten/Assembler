...
Although it was still a pretty functional program, I was incredibly dissatisfied with the end result. I would've liked to program a more polished version, but unfortunately I had to step away from it to address other assignments and projects.

Then came Spring semester of my sophomore year. I noticed that the class was accepting a TA position. Since I had done very well the year prior, I decided to take the position.
I was accepted into the role and had office hours dedicated to helping those who were struggling. When the assignment came up again, this time for the current students, I had an overwhelming desire to try again.
...
5. I engineered the overall and file-based architecture to act similarly to a DAG (directed acyclic graph). This decision means that no two files and no two functions are dependent upon each other.
6. I gave functions clear names and purposes.
7. I used tools such as GDB and Valgrind to debug code and find memory leaks.

There were more aspects I worked to improve on, but these were the core principles I wanted to establish when engineering my solution. Of course, there are multiple sections that can still be improved. However, in comparison to my original submission, the quality was exponentially greater.

When I wrote the first assembler I had so many memory leaks. With the new assembler, I was able to spot and fix all possible memory issues: There are no memory leaks that I have found in my new solution.
When I wrote the first assembler I had so many bugs that I couldn't find the source of. With my new mentality, I was able to isolate and exterminate any bugs in my code with ease.
When I wrote the first assembler I could barely tell what I had written the day before. With the way I now structure my code and name my functions, I knew what I meant in almost every snippet of code.
